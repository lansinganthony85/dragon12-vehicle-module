...
; export symbols
            ...
            XDEF noise_init, start_sound, stop_sound
...
;  CUSTOM
;   noise_init
;   We prepare the TC5 and TC7 channels for sending a pulse train
;   through the speaker to make sound; however, we will not be
;   turning on the pulse train until sound is needed as this would
;   cause the speaker to pulse even when sound is off as we use the
;   timer for the full operation of the vehicle
noise_init:
            movb  #$A0,TIOS   ;select output compares 5 & 7
            movb  #$04,TSCR2  ;div by 16: 24MHz/16 = 1.5 MHz
            movb  #$80,TSCR1  ;enable timer
...
; CUSTOM
;  start_sound()
; Since our design uses the timer for the full operation of the vehicle
; We only set up the TC5 and TC7 channels for sound when we need them.
; Otherwise, the speaker will be pulsing even when we turn sound off.
start_sound:
            bset  OC7M,#$20   ;pulse train out PT5
            bset  OC7D,#$20   ;PT5 goes high on TC7 match
            bset  TCTL1,#$08  ;PT5 low on TC5  match
...
; CUSTOM
;  stop_sound()
; only disable the interrupts that make sound
; turn off the pulse train so that the speaker will not
